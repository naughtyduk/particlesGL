...
const particlesGL = (function () {
  ...
  async function loadModelGeometry(modelSrc, options) {
    return new Promise((resolve, reject) => {
      if (!GLTFLoader) {
        console.error("particlesGL: GLTFLoader is required for model loading.");
        return reject("Missing GLTFLoader. Required for 3D model support.");
      }
   ...
   *  Public API
   * ------------------------------------------------*/
  const particlesGL = function (userOptions = {}) {
    const defaults = {
      target: ".particlesGL",
      character: "•",
      particleSize: 0.015,
